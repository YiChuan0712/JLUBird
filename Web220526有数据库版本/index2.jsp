            ...
            if ($('#chartjs_pie').length) {
                ...
                var myChart = new Chart(ctx, {
                    ...
                    data: {
						...
						//labels: ["M", "T",],
						out.println("labels: [");
						PreparedStatement pStmt = conn.prepareStatement("select * from bcount order by count desc");
													ResultSet rs = pStmt.executeQuery();
													while(rs.next())
													{
														String nm = rs.getObject("name").toString();
														out.println("\""+nm+"\",");
													}
						out.println("],");
						%>
                            ...
                            backgroundColor: [
                               "#5969ff",
								...
								"#0088cc",
													...
													//data: [12, 19,]
						Class.forName("com.mysql.jdbc.Driver");
						String url1 = "jdbc:mysql://localhost:3306/bird?serverTimezone=UTC";
						String usernam1 = "root";  //数据库用户名
						String password1 = "root";  //数据库用户密码
						Connection conn1 = DriverManager.getConnection(url1, usernam1, password1);  //连接状态
						//labels: ["M", "T",],
						out.println("data: [");
						PreparedStatement pStmt1 = conn1.prepareStatement("select * from bcount order by count desc");
													ResultSet rs1 = pStmt1.executeQuery();
													while(rs1.next())
													{
														String cnt = rs1.getObject("count").toString();
														out.println(cnt+",");
													}
						out.println("]");
						%>
            ...
            new Chartist.Line('.ct-chart-polar', {
                labels: [1, 2, 3, 4, 5, 6, 7, 8],
                   ...
                   // ]
											...
											try {
												Class.forName("com.mysql.jdbc.Driver");
												String url2 = "jdbc:mysql://localhost:3306/bird?serverTimezone=UTC";
												String usernam2 = "root";  //数据库用户名
												String password2 = "root";  //数据库用户密码
												Connection conn2 = DriverManager.getConnection(url2, usernam2, password2);  //连接状态

												if (conn2 != null) 
												{
													//usernam = request.getParameter("username");
													//password = request.getParameter("password");
													PreparedStatement pStmt2 = conn2.prepareStatement("select * from ctable order by f");
													ResultSet rs2 = pStmt2.executeQuery();
													int i = 0;
													while(rs2.next())
													{
														String b1 = rs2.getObject("b1").toString();
														String b2 = rs2.getObject("b2").toString();
														String b3 = rs2.getObject("b3").toString();
